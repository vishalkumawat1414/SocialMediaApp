...
const getFeedData = async (req, res) => {
	try {
		...
		//posts of following(feed)
		const posts = fullPost?.map((item) => mapPostOutput(item, req._id));
		//for suggestion
		const followingsId = curUser.followings.map((item) => item._id); //id of all the followings
...
const getUserProfile = async (req, res) => {
	try {
		...
		// console.log("from controlller",fullPosts)
